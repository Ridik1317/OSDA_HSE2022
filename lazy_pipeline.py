...
def predict_with_max_generators(
    ...
    max_pos_intsec_len = 1
    max_pos_intsec = X_pos[0]
    for x_pos in X_pos:
        intersection_pos = x & x_pos
        if len(intersection_pos) > max_pos_intsec_len:
          max_pos_intsec_len = len(intersection_pos)
          max_pos_intsec = intersection_pos
      
    for x_pos in X_pos:  # count all negative examples that contain intersection_pos
        if (max_pos_intsec & x_pos) == max_pos_intsec:
            n_counters_pos += 1

    max_neg_intsec_len = 1
    max_neg_intsec = X_neg[0]
    n_counters_neg = 0  # number of counter examples for negative intersections
    for x_neg in X_neg:
        intersection_neg = x & x_neg
        if len(intersection_neg) > max_neg_intsec_len:
          max_neg_intsec_len = len(intersection_neg)
          max_neg_intsec = intersection_neg

    for x_neg in X_neg:  # count all positive examples that contain intersection_neg
        if (max_neg_intsec & x_neg) == max_neg_intsec:
            n_counters_neg += 1

    if max_neg_intsec_len != max_pos_intsec_len:
      prediction = max_neg_intsec_len < max_pos_intsec_len
      return prediction
    else:
      perc_counters_pos = n_counters_pos / len(X_pos)
      perc_counters_neg = n_counters_neg / len(X_neg)
      prediction = perc_counters_neg < perc_counters_pos
      return prediction
